...
class MockMetaWeatherApiClient extends Mock implements meta_weather_api.MetaWeatherApiClient {}
  ...
  group('WeatherRepository', () {
    ...
    group('getWeather', () {
      ...
      test('throws when locationSearch fails', () async {
        ...
        expect(() async => await weatherRepository.getWeather(city), throwsA(exception));
      });

      test('calls getWeather with correct woeid', () async {
        final location = MockLocation();
        when(() => location.woeid).thenReturn(woeid);
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((invocation) async => location);
        try {
          await weatherRepository.getWeather(city);
        } catch (e) {}
      ...
      test('returns correct weather on success (showers)', () async {
        ...
        when(() => weather.weatherStateAbbr).thenReturn(meta_weather_api.WeatherState.showers);
        when(() => weather.theTemp).thenReturn(42.42);
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((invocation) async => location);
        when(() => metaWeatherApiClient.getWeather(any())).thenAnswer((invocation) async => weather);
      ...
      test('returns correct weather on success (heavy cloud)', () async {
        ...
        when(() => location.woeid).thenReturn(woeid);
        when(() => location.title).thenReturn('London');
        when(() => weather.weatherStateAbbr).thenReturn(meta_weather_api.WeatherState.heavyCloud);
        when(() => weather.theTemp).thenReturn(42.42);
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((invocation) async => location);
        when(() => metaWeatherApiClient.getWeather(any())).thenAnswer((invocation) async => weather);
        final actual = await weatherRepository.getWeather(city);
      ...
      test('returns correct weather on success (light cloud)', () async {
        ...
        when(() => location.woeid).thenReturn(woeid);
        when(() => location.title).thenReturn('London');
        when(() => weather.weatherStateAbbr).thenReturn(meta_weather_api.WeatherState.lightCloud);
        when(() => weather.theTemp).thenReturn(42.42);
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((invocation) async => location);
        when(() => metaWeatherApiClient.getWeather(any())).thenAnswer((invocation) async => weather);
        final actual = await weatherRepository.getWeather(city);
