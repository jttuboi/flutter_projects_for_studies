...
void main() {
  ...
  // pelo que parece, são os dados que o Firebase emite, aqui são os dados fictícios
  ...
  group('AuthenticationRepository', () {
    ...
    setUp(() {
      ...
      authenticationRepository = AuthenticationRepository(cache: cache, firebaseAuth: firebaseAuth, googleSignIn: googleSignIn);
    });
    ...
    group('signUp', () {
      setUp(() {
        when(() {
          return firebaseAuth.createUserWithEmailAndPassword(email: any(named: 'email'), password: any(named: 'password'));
      ...
      test('throws SignUpWithEmailAndPasswordFailure when createUserWithEmailAndPassword throws', () async {
        when(() => firebaseAuth.createUserWithEmailAndPassword(email: any(named: 'email'), password: any(named: 'password'))).thenThrow(Exception());
        expect(authenticationRepository.signUp(email: email, password: password), throwsA(isA<SignUpWithEmailAndPasswordFailure>()));
      });
    });
    ...
    group('loginWithGoogle', () {
      ...
      test('throws LogInWithGoogleFailure and calls signIn authentication, and signInWithPopup when authCredential is null and kIsWeb is true',
          () async {
        authenticationRepository.isWeb = true;
        await expectLater(() => authenticationRepository.logInWithGoogle(), throwsA(isA<LogInWithGoogleFailure>()));
        verifyNever(() => googleSignIn.signIn());
        verify(() => firebaseAuth.signInWithPopup(any())).called(1);
      });

      test('sucessfully calls signIn authentication, and signInWithPopup when authCredential is not null and kIsWeb is true', () async {
        ...
        authenticationRepository.isWeb = true;
        await expectLater(authenticationRepository.logInWithGoogle(), completes);
        verifyNever(() => googleSignIn.signIn());
        verify(() => firebaseAuth.signInWithPopup(any())).called(1);
      });
      ...
      test('throws LogInWithGoogleFailure when exception occurs', () async {
        ...
        expect(authenticationRepository.logInWithGoogle(), throwsA(isA<LogInWithGoogleFailure>()));
      });
    });
    ...
    group('logInWithEmailAndPassword', () {
      setUp(() {
        when(() {
          return firebaseAuth.signInWithEmailAndPassword(email: any(named: 'email'), password: any(named: 'password'));
        }).thenAnswer((_) => Future.value(MockUserCredential()));
      });
      ...
      test('succeeds when signInWithEmailAndPassword succeeds', () async {
        expect(authenticationRepository.logInWithEmailAndPassword(email: email, password: password), completes);
      });

      test('throws LogInWithEmailAndPasswordFailure when signInWithEmailAndPassword throws', () async {
        when(() => firebaseAuth.signInWithEmailAndPassword(email: any(named: 'email'), password: any(named: 'password'))).thenThrow(Exception());
        expect(
    ...
    group('user', () {
      test('emits User.empty when firebase user is null', () async {
        ...
        await expectLater(authenticationRepository.user, emitsInOrder(const <User>[User.empty]));
      });
      ...
      test('emits User when firebase user is not null', () async {
        ...
        await expectLater(authenticationRepository.user, emitsInOrder(const <User>[user]));
