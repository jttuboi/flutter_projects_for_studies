  ...
  group('PostBloc', () {
    ...
    group('PostFetched', () {
      ...
      blocTest<PostBloc, PostState>(
        ...
        setUp: () {
          when(() => httpClient.get(any())).thenAnswer((_) async {
            return http.Response('[{ "id": 1, "title": "post title", "body": "post body" }]', 200);
          });
        },
        build: () => PostBloc(httpClient: httpClient),
        act: (bloc) => bloc.add(PostFetched()),
      ...
      blocTest<PostBloc, PostState>(
        ...
        build: () => PostBloc(httpClient: httpClient),
        act: (bloc) => bloc.add(PostFetched()),
        expect: () => [const PostState(status: PostStatus.failure)],
        verify: (bloc) {
          verify(() => httpClient.get(_postsUrl(start: 0))).called(1);
        },
      );
      ...
      blocTest<PostBloc, PostState>(
        'emits successful status and reaches max posts when 0 additional posts are fetched',
        setUp: () {
      ...
      blocTest<PostBloc, PostState>(
        'emits successful status and does not reach max posts when additional posts are fetched',
        setUp: () {
          when(() => httpClient.get(any())).thenAnswer((invocation) async {
            return http.Response('[{ "id": 2, "title": "post title", "body": "post body" }]', 200);
          });
        },
        build: () => PostBloc(httpClient: httpClient),
        seed: () => const PostState(
